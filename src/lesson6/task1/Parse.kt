...
fun bestLongJump(jumps: String): Int {
    ...
    var maxValue = -1
    for (element in list) {
        if (!element.matches(format)) return -1
        val elementInt = element.toIntOrNull() ?: continue
        if (elementInt > maxValue) maxValue = elementInt
    }
    return maxValue
}
...
fun bestHighJump(jumps: String): Int {
    val format = Regex("""\d+\s[%\-]*\+[%\-]*""").findAll(jumps)
    var valueList: List<String>
    var max = -1
    for (x in format) {
        valueList = x.value.split(Regex("""\s.+"""))
...
fun firstDuplicateIndex(str: String): Int {
    ...
    for (i in splitList.indices) {
        val pairValue = Pair(splitList[i].toLowerCase(), startIndex)
        startIndex += splitList[i].length + 1
        list.add(pairValue)
    }
    if (list.size < 2) return -1
    for (k in 1..list.lastIndex) {
        if (list[k].first == list[k - 1].first) return list[k - 1].second
    }
    return -1
}
