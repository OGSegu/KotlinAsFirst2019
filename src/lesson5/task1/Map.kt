...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>) {
    ...
    for ((key, value) in a) {
        if (b[key] == value) keyList.add(key)
    }
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for (key in keySet) {
        ...
        if (mapA.containsKey(key)) valueSet.add(mapA.getValue(key))
        if (mapB.containsKey(key)) valueSet.add(mapB.getValue(key))
...
fun hasAnagrams(words: List<String>): Boolean {
    val resultSet = mutableSetOf<Set<Char>>()
    val lengthGroup = words.map { it.toList() }.groupBy { it.size }
    for (element in lengthGroup.values) {
        val size = element.size
        for (index in element) {
            resultSet.add(index.toSet())
        }
        if (resultSet.size != size) return true
        resultSet.clear()
    }
    return false
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        if (number - list[i] in valueMap) {
            return Pair(valueMap[number - list[i]]!!, i)
        }
        valueMap[list[i]] = i
    }
    return Pair(-1, -1)
